...
use amm::tests::helpers::actions::{token::{deploy_token, fund, approve}, quoter::deploy_quoter};
...
fn before() -> (
    IMarketManagerDispatcher, IERC20Dispatcher, IERC20Dispatcher, IERC20Dispatcher, IQuoterDispatcher
) {
    // Get default owner.
    let owner = owner();
...
fn test_quote_multiple() {
    ...
    let quote = quoter.quote_multiple(
