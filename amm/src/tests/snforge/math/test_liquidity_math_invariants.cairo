...
#[test]
fn test_liquidity_to_base_invariants(
    liquidity: u128,
    width: u16, 
    rand1: u32,
    rand2: u32
    ...
    let base_round_down = liquidity_to_base(lower_sqrt_price, upper_sqrt_price, liquidity_i256, false);
    let base_round_up = liquidity_to_base(lower_sqrt_price, upper_sqrt_price, liquidity_i256, true);
    assert(base_round_down <= base_round_up, 'Base rnd down <= base rnd up');

    if liquidity == 0 { return; }
    let liquidity_deriv = base_to_liquidity(lower_sqrt_price, upper_sqrt_price, base_round_down.val);
...
#[test]
fn test_liquidity_to_quote_invariants(
    liquidity: u128,
    width: u16, 
    rand1: u32,
    rand2: u32
    ...
    let quote_round_down = liquidity_to_quote(lower_sqrt_price, upper_sqrt_price, liquidity_i256, false);
    let quote_round_up = liquidity_to_quote(lower_sqrt_price, upper_sqrt_price, liquidity_i256, true);
    assert(quote_round_down <= quote_round_up, 'Quote rnd down <= base rnd up');

    if liquidity == 0 { return; }
    let liquidity_deriv = quote_to_liquidity(lower_sqrt_price, upper_sqrt_price, quote_round_down.val);
    assert(liquidity_deriv <= liquidity.into(), 'Liquidity deriv <= liquidity');
}