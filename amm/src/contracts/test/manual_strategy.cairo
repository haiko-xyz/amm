...
mod ManualStrategy {
    ...
    impl Strategy of IStrategy<ContractState> {
        ...
        fn queued_positions(self: @ContractState, market_id: felt252) -> Span<PositionInfo> {
            ...
            let (bid_base, bid_quote, bid_base_fees, bid_quote_fees) = market_manager.amounts_inside_position(bid_pos_id);
            let (ask_base, ask_quote, ask_base_fees, ask_quote_fees) = market_manager.amounts_inside_position(ask_pos_id);

            // Calculate liquidity.
            let width = market_manager.width(market_id);
            let quote_amount = quote_reserves + bid_quote + ask_quote + bid_quote_fees + ask_quote_fees;
